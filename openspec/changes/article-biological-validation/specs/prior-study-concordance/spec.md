...
The analysis MUST compare the ~226 gained DEGs (present in softImpute result but absent from intersection-only result, using 6ds data) against the full limma table from Lykhenko 2021a (`output/dissertation/integrative_pipeline_disser/1_2/difexp/difexp_all.csv`, 20,162 genes, 4 Affy datasets, 22 samples). This is the strongest rebuttal to the "imputation artefact" concern.

#### Scenario: Concordance categorization

Given the ~226 gained DEGs and the 2021 full limma output
...
When logFC from the current 6-dataset study is plotted against logFC from the 2021 4-Affy study
...
Then the gained DEGs found in 2021 are highlighted in colour
...
Given the DEG lists from: (a) this study softImpute 447, (b) this study intersection-only 221, (c) Lykhenko 2021a 327
