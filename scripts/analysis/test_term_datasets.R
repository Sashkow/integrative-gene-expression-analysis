...
# Setup logging - capture all output to file
log_dir <- "output/test_term_datasets"
dir.create(log_dir, showWarnings = FALSE, recursive = TRUE)
log_file <- file.path(log_dir, paste0("run_log_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".txt"))
sink(log_file, split = TRUE)  # split=TRUE outputs to both console and file
...
# Load configuration
config <- load_config("config/config_test_term_datasets.yaml")

# Create output directory for test iterations
test_output_dir <- "output/test_term_datasets"
...
addon_datasets <- config$term_datasets$datasets
global_exclusions <- config$global_exclusions$datasets

cat("Baseline datasets:", paste(baseline_datasets, collapse = ", "), "\n")
cat("Additional datasets to test:", paste(addon_datasets, collapse = ", "), "\n")
cat("Global exclusions:", paste(global_exclusions, collapse = ", "), "\n\n")
...
# Exclude all addon datasets and global exclusions - we only want the baseline datasets
...
n_term <- sum(pdata_filtered$Gestational.Age.Category == "Term", na.rm = TRUE)

cat("Samples: Second Trimester =", n_second, ", Term =", n_term, "\n")
...
for (test_datasets in datasets_to_test) {
  ...
  # Also always exclude global_exclusions
  other_addon_datasets <- setdiff(addon_datasets, test_datasets)
  exclusions <- c(other_addon_datasets, global_exclusions)
  ...
  if (n_term == 0) {
    cat("⚠ No Second Trimester samples for", test_name, "- skipping\n")
    next
  }
  ...
  tryCatch({
    ...
    if (!is.null(baseline_genes_merged)) {
      ...
      if (length(genes_gained_vec) > 0) {
        ...
        # Save to file
        gained_file <- file.path(test_output_dir, paste0("term_", gsub("\\+", "_", test_name), "_genes_gained.txt"))
      ...
      if (length(genes_lost_vec) > 0) {
        ...
        # Save to file
        lost_file <- file.path(test_output_dir, paste0("term_", gsub("\\+", "_", test_name), "_genes_lost.txt"))
...
# Identify datasets with both Second Trimester and Term samples
...
datasets_with_both <- rownames(dataset_counts)[
  dataset_counts[, "Second Trimester"] >= 3 & dataset_counts[, "Term"] >= 3
]

cat("Datasets with both Second Trimester and Term (>=3 each):\n")
...
for (single_ds in datasets_with_both) {
  ...
  n_term <- sum(pdata_single$Gestational.Age.Category == "Term", na.rm = TRUE)
  ...
  tryCatch({
    ...
    difexp_file <- file.path(test_output_dir, paste0("term_", single_ds, "_single_difexp.csv"))
    write.csv(difexp_logfc1, difexp_file, row.names = FALSE)
    cat("Saved DEG results to:", difexp_file, "\n")

    # Save unfiltered differential expression results (all genes)
    difexp_file_unfiltered <- file.path(test_output_dir, paste0("term_", single_ds, "_single_difexp_unfiltered.csv"))
...
if (nrow(single_results) > 0) {
  ...
  # Save summary
  single_summary_file <- file.path(test_output_dir, "term_single_dataset_summary.csv")
...
cat("\nBaseline total: Second Trimester =", sum(baseline_table[, "Second Trimester"]),
    ", Term =", sum(baseline_table[, "Term"]), "\n\n")
...
print(all_table)
cat("\nTotal: Second Trimester =", sum(all_table[, "Second Trimester"]),
    ", Term =", sum(all_table[, "Term"]), "\n\n")
...
if (nrow(results_summary) > 0) {
  ...
  with_addons <- results_summary[results_summary$type == "with_term", ]
