#!/usr/bin/env Rscript
# Side-by-side scatters for Soncin leave-one-out (full Soncin, all weeks)
# A: GSE100051 single (all weeks) vs 7ds_no_soncin
# B: 8ds (original) vs 7ds_no_soncin
...
n_total <- nrow(pheno)
n_son <- sum(pheno$dataset_id == "GSE100051")
n_7s  <- n_total - n_son

# --- Plot A: GSE100051 single (all weeks) vs 7ds_no_soncin ---
merged_a <- merge(
...
# --- Plot B: 8ds (original) vs 7ds_no_soncin ---
merged_b <- merge(
...
pa <- ggplot(df_a, aes(x = logFC_single, y = logFC_7s, color = sig_cat)) +
  ...
  labs(title = "A. GSE100051 single vs 7ds no Soncin",
       x = sprintf("GSE100051 logFC (%d samples)", n_son),
...
pb <- ggplot(df_b, aes(x = logFC_8ds, y = logFC_7s, color = sig_cat)) +
  ...
  labs(title = "B. 8ds vs 7ds no Soncin",
       x = sprintf("8ds logFC (%d samples)", n_total),
       y = sprintf("7ds no Soncin logFC (%d samples)", n_7s)) +
  theme_minimal(base_size = 13) +
  theme(plot.title = element_text(face = "bold", size = 13),
        legend.position = "inside",
...
img_path <- file.path(output_dir,
  "scatter_soncin_full_vs_no_soncin_side_by_side.png")
...
caption_a <- sprintf(paste0(
  "A. GSE100051 single-dataset (limma, %d samples, %d DEGs) vs ",
...
caption_b <- sprintf(paste0(
  ...
  "Pearson r = %.3f, Spearman rho = %.3f, same direction = %.1f%%."),
  n_total, n_deg_8ds, n_7s, n_deg_7s,
...
docx_path <- file.path(output_dir, "scatter_soncin_full_vs_no_soncin.docx")
doc <- read_docx()
