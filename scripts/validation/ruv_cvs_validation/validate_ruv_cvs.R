#!/usr/bin/env Rscript
#' Validate RUV k=1 correction for CVS vs abortion comparison.
#'
...
#'
#' Usage: Rscript validate_ruv_cvs.R [n_permutations]
...
out_dir <- "output/validation/ruv_cvs_validation"
...
abort_expr <- read.delim("data/mapped/GSE93520.tsv",
                          row.names = 1, check.names = FALSE)

pheno <- read.csv("data/phenodata/samples.csv", stringsAsFactors = FALSE)
...
abort_pheno <- pheno[pheno$secondaryaccession == "GSE93520" &
                      pheno$Diagnosis == "Healthy" &
                      pheno$Gestational.Age %in% c("9", "10"), ]
...
control_genes <- readLines(
  "scripts/integrative_analysis/phase2b_direct_merge/config_ruv_cvs/ruv_control_genes_cvs.txt")
control_idx <- which(rownames(merged) %in% control_genes)

cat(sprintf("Merged matrix: %d genes x %d samples (%d CVS, %d abortion)\n",
            nrow(merged), ncol(merged), n_cvs, n_abort))
cat(sprintf("Control genes in matrix: %d\n", length(control_idx)))

# ── Helper: run RUV k=1 + limma for a given group vector ─────────────────────
...
real_de <- run_ruv_limma(as.matrix(merged), real_group, control_idx, k = 1)
...
for (i in seq_len(n_perm)) {
  ...
  perm_de <- run_ruv_limma(merged_mat, perm_labels, control_idx, k = 1)
...
hist(perm_degs, breaks = 30, col = "grey80", border = "grey50",
     main = sprintf("Permutation test: RUV k=1 CVS vs Abortion (%d permutations)", n_perm),
...
# Direction-specific permutation plot
...
hist(real_de$P.Value, breaks = 50, col = "steelblue", border = "white",
     main = "P-value distribution (RUV k=1, CVS vs Abortion)",
...
# P-value comparison between control and non-control
...
# KS test: are control gene p-values more uniform than non-control?
...
cat("\n")
cat("════════════════════════════════════════════════════════════════\n")
cat("  RUV k=1 Validation Summary\n")
cat("════════════════════════════════════════════════════════════════\n")
...
cat(sprintf("RUV k=1 CVS vs Abortion Validation\n"))
cat(sprintf("Date: %s\n\n", Sys.time()))
cat(sprintf("Samples: %d CVS (GSE12767) vs %d abortion (GSE93520, GA 9-10)\n", n_cvs, n_abort))
