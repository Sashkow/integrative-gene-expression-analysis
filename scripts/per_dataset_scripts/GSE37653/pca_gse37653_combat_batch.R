...
plot_dir <- "output/one_off/pca_3_yehor_gse37653/combat_batch"
...
cat("\nSex distribution per batch:\n")
print(table(pdata$Batch, pdata$fetux_sex_estimate))

batch <- as.factor(pdata$Batch)
sex <- as.factor(pdata$fetux_sex_estimate)
mod <- model.matrix(~ sex)

cat("\nRunning ComBat (no ref.batch, 4 batches, sex covariate)...\n")
...
write.table(exprs_combat, file.path(output_dir, "GSE37653_entrez_protein_coding_enriched_sashko.tsv"),
            sep = "\t", quote = FALSE)
cat("\nCorrected matrix saved to:", file.path(output_dir, "GSE37653_entrez_protein_coding_enriched_sashko.tsv"), "\n")
